...
function parseWorkspace(value: unknown, path: string): Workspace {
  ...
  const slug = patternedString(input.slug, `${path}.slug`, 1, 63, WORKSPACE_SLUG_PATTERN)
  return {
    ...
    plan: closedStringEnum(input.plan, `${path}.plan`, ['personal', 'team', 'enterprise']),
    slug
  }
}
...
function parseWorkspaceAccess(value: unknown, path: string): WorkspaceAccess {
  ...
  return {
    ...
    role: closedStringEnum(input.role, `${path}.role`, ['owner', 'admin', 'editor', 'viewer']),
...
export function parseWorkspaceList(value: unknown): CursorCollection<WorkspaceAccess> {
  ...
  const items = arrayBetween(input.items, 'workspace_list.items', 0, 200)
  return {
