...
export function createProductionContentSecurityPolicy(
  ...
  return `default-src 'self'; base-uri 'self'; object-src 'none'; frame-src ${API_V2_PRODUCTION_ORIGIN}; form-action 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; font-src 'self' data:; connect-src ${uniqueConnectSources}; media-src 'self' blob:; worker-src 'self' blob:`
}
