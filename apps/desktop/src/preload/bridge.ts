import { RUNTIME_INFO_CHANNEL, SAVE_ARTIFACT_CHANNEL } from '@ai-job-workspace/platform'
import type {
  PlatformBridge,
  RuntimeInfo,
  SaveArtifactRequest,
...
export function createDesktopPlatformBridge(
   ...
   * @param request 只含产物 URL 与安全文件名的请求 / Request containing only the artifact URL and safe filename.
  ...
  function saveArtifact(request: SaveArtifactRequest): Promise<SaveArtifactResult> {
    return invokeArtifactSave(SAVE_ARTIFACT_CHANNEL, request)
  }
