...
| PDF 下载地址 | 契约把 `download_url` 定义为短期签名 URI，但未约定可用 origin、重定向边界、浏览器 CSP/CORS 与桌面主进程的信任清单。 | 当前只接受已验证产品 API origin 下的 artifact content 路径；若服务端改用对象存储/CDN，须先冻结并部署 origin allowlist，不能让主进程抓取任意后端返回的 URL。 |
...
| 评估量表     | 历史 `rubric_ref` 对应 rubric 的查询端点与分数尺度待确定。                                                          | 仅按后端报告返回的量表字段展示，不在前端换算或生成百分制分数。                                                                                              |
