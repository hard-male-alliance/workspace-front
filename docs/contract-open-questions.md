...
本文件不定义、替代或修改 `contract/` 中的正式前后端契约。当前前端只装配名字明确的 `Mock*Gateway`，所有条目在服务端契约冻结前都不得被当作可调用接口。
...
| 范围         | 待确认项                                                                                                | v0.1 前端处理                                                |
| ------------ | ------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| HTTP DTO     | 多数列表、创建、更新端点在 Schema 中没有逐端点 request/response entrypoint。                            | 仅调用 mock gateway，不创建假 `fetch` 客户端。               |
...
| PDF 预览     | 产物签名 URL、CORS、过期刷新与 PDF viewer 策略未定义。                                                  | 使用纯前端 mock 视觉预览，不把它误标为 PDF 产物。            |
