...
Resume 文档写入在展示层与 adapter 均按 Resume ID 使用单通道互斥，不排队、不自动重放旧 revision 意图。当前 pinned 模板的样式更新只发送 `set_style_intent`；compatibility 与 migration Job 契约冻结前，模板目录只读，不保留一个永远失败的“切换”端口。
