...
Web 在组合根配置失败时渲染不依赖业务 i18n 的可恢复启动错误；Electron main 先显示脱敏的本地化原生错误框，再以非零状态结束不完整进程。两者都不在配置失败时切换到第二套数据组合。
...
异步资源状态同时绑定领域资源 key、loader 与 retry attempt。路由从 A 切换到 B 的当前 render 立即进入 loading，迟到的 A 响应不能提交；带本地草稿或弹窗的子树再以领域 ID 作为 React key 重建，避免向错误聚合根写入。

在线 Resume 文档写入在展示层与 adapter 均按 Resume ID 使用单通道互斥，不在内存中排队，也不自动重放旧 revision 意图。这不是契约所要求的 Electron 持久离线队列实现；在 native 身份与登出生命周期冻结前，桌面端不得把 operation batch 跨进程落盘后宣称支持离线编辑。当前 pinned 模板的样式更新只发送 `set_style_intent`；compatibility 与 migration Job 契约冻结前，模板目录只读，不保留一个永远失败的“切换”端口。
...
### 5. 身份、Electron 离线队列与跨源 artifact 是上游准入条件
...
- Electron 持久 operation queue 的稳定用户主体、登出清理与凭据失效边界；
- artifact origin、每跳 redirect allowlist、CSP/CORS 与认证头传播规则。

在跨源策略冻结前，PDF 预览只接受同 API origin、精确 artifact ID/path 且声明 `application/pdf` 的 URL，并使用 sandboxed iframe。保存端口只接收 artifact ID 与安全文件名；Web、preload 和 Electron main 在各自信任边界重复调用同一纯 decoder，不传递“已验证”权限。Web adapter 和 Electron main 都会在保存时重新读取完整权威 metadata，不信任 renderer 传来的 URL、`size_bytes` 或 `sha256`。两种宿主的 metadata 与 content Fetch 均使用 `redirect: 'error'` 拒绝任何重定向；只有上游冻结逐跳 allowlist、认证传播和宿主 transport 后才允许改变这一 fail-closed 策略。Electron 保存端在流式核对实际字节数与 SHA-256 成功后才 `fsync` 并原子替换目标文件。Web 端在 25 MiB 上限内读取 PDF，核对媒体类型、实际大小与 SHA-256 后才通过一次性 Blob URL 启动下载。Fetch 会解码受支持的 `Content-Encoding`，因此压缩响应的传输 `Content-Length` 不用于校验解码字节；CORS 隐藏编码 header 时同样忽略该提前检查，最终实际字节、25 MiB 上限与 SHA-256 校验不受影响。浏览器仍无法观察之后的文件系统写入，因此只报告已启动，不宣称已保存。后端仍必须返回正确的 `Content-Type`、`X-Content-Type-Options: nosniff`，并在生产启用前冻结 Bearer 与 CORS 传播。
...
这些输入冻结后才能实现生产 token supplier、按用户隔离的 Electron operation outbox 与跨源下载策略；两种宿主不能共用一套假定的浏览器身份实现。离线队列必须由 main 进程拥有并加密存储，按 API origin、用户与 workspace 分区，重放时保留原 operation ID、base revision 和命令身份；409/412 必须停止队列并读取权威版本。操作系统安全存储不可用时必须关闭离线队列，Linux 上不得接受 Electron `safeStorage` 的 `basic_text` 后端作为加密保证。
...
- [React: Preserving and Resetting State](https://react.dev/learn/preserving-and-resetting-state) 将领域身份作为 `key` 来重建不应跨资源保留的本地状态。
- [Fetch Standard](https://fetch.spec.whatwg.org/) 规定 response body 按 content coding 解码；应用不能用压缩传输长度校验解码后字节。
- [Electron Security](https://www.electronjs.org/docs/latest/tutorial/security) 要求 context isolation、sandbox、导航/新窗口限制与 IPC sender 校验共同成为安全边界。
- [Electron safeStorage](https://www.electronjs.org/docs/latest/api/safe-storage) 说明各平台使用的系统密钥提供方，并明确 Linux `basic_text` 后端不提供可信的磁盘加密保证。
...
用户看到的资源、成功状态和错误说明都有明确权威来源；未知结果不会被包装成成功。在当前页面与应用实例生命周期内，同一用户意图会复用稳定命令身份，避免确认操作意外创建新的 Session 或 Render Job；renderer 重启后的恢复仍需要持久命令日志或 provider reconciliation，当前实现不作跨进程保证，也不宣称支持 Electron 离线编辑。未实现能力不会被包装成服务端数据。代价是身份、桌面离线队列、实时媒体、上传和跨源 artifact 在上游契约冻结前保持不可用，但这种不可用是可审计的真实状态，而不是演示数据留下的隐性分支。
